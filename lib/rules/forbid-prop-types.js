    ...
    function isPropTypesPackage(node) {
      return (
        (node.type === 'Identifier' &&
          (node.name === null || node.name === propTypesPackageName || !isForeignPropTypesPackage)) ||
        (node.type === 'MemberExpression' &&
          (node.object.name === null || node.object.name === reactPackageName || !isForeignPropTypesPackage))
      );
    }
    ...
    return {
      ...
      CallExpression(node) {
        ...
        if (
          ...
          (('name' in node.callee && node.callee.name === 'shape') || astUtil.getPropertyName(node.callee) === 'shape')
        ) {
