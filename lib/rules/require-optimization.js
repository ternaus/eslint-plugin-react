  ...
  create: Components.detect((context, components) => {
    ...
    return {
      ...
      ClassDeclaration(node) {
        if (
          !(hasPureRenderDecorator(node) || hasCustomDecorator(node) || componentUtil.isPureComponent(node, context))
        ) {
          return;
        }
        markSCUAsDeclared(node);
      ...
      ObjectExpression(node) {
        ...
        const found = node.properties.some(
          (property) => property.key && (isSCUDeclared(property.key) || isPureRenderDeclared(property)),
        );
        if (found) {
          markSCUAsDeclared(node);
        }
