...
const exported = {
  ...
  create(context) {
          ...
          if (jsxUtil.isJSX(expression)) {
            ...
            if (parentType === 'JSXAttribute') {
              if (expressionType !== 'TemplateLiteral' && /["]/.test(expression.raw.slice(1, -1))) {
                ...
                textToReplace = `"${
                  expressionType === 'TemplateLiteral' ? expression.quasis[0].value.raw : expression.raw.slice(1, -1)
                }"`;
              }
              ...
              textToReplace =
                expressionType === 'TemplateLiteral' ? expression.quasis[0].value.cooked : expression.value;
            }
          }
    ...
    function areRuleConditionsSatisfied(parent, config, ruleCondition) {
      return (
        (parent.type === 'JSXAttribute' && typeof config.props === 'string' && config.props === ruleCondition) ||
        (jsxUtil.isJSX(parent) && typeof config.children === 'string' && config.children === ruleCondition)
      );
    }
    ...
    function emptyStringExpressionPreservesWhitespace(node, children) {
      ...
      return (
        ...
        hasAdjacentJsx(
          node,
          children.filter((child) => child !== previous),
        )
      );
    }
