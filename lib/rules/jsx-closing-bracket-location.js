...
const getSourceCode = requiredModule0.getSourceCode;
  ...
  create(context) {
    ...
    function hasCorrectLocation(tokens, expectedLocation) {
      switch (expectedLocation) {
        ...
        case 'after-props':
          return (
            (tokens.lastComment ? tokens.lastComment.loc.end.line : tokens.lastProp.lastLine) === tokens.closing.line
          );
        case 'props-aligned':
        case 'tag-aligned':
        case 'line-aligned': {
    ...
    function getTokensLocations(node) {
      ...
      if (node.attributes.length) {
        ...
        const commentsAfterLastAttribute = sourceCode
          .getCommentsAfter(lastAttribute)
          .filter((comment) => comment.range[1] <= closingToken.range[0]);
        lastComment = commentsAfterLastAttribute.at(-1);
      } else {
        const commentsAfterTag = sourceCode
          .getCommentsAfter(node.name)
          .filter((comment) => comment.range[1] <= closingToken.range[0]);
        lastComment = commentsAfterTag.at(-1);
      }
      ...
      'JSXOpeningElement:exit'(node) {
        ...
        report(context, messages.bracketLocation, 'bracketLocation', {
          ...
          fix(fixer) {
            ...
            const lastRelevantEndPos = tokens.lastComment?.range[1] ?? cachedLastAttributeEndPos;
            switch (expectedLocation) {
              case 'after-tag':
              ...
              case 'line-aligned':
                return fixer.replaceTextRange(
                  [lastRelevantEndPos, node.range[1]],
