...
function hasExternalLink(node, linkAttributes, warnOnSpreadAttributes, spreadAttributeIndex) {
  const linkIndex = node.attributes.findLastIndex((attr) => attr.name && linkAttributes.includes(attr.name.name));
...
function hasDynamicLink(node, linkAttributes) {
  const dynamicLinkIndex = node.attributes.findLastIndex(
    (attr) =>
      attr.name &&
...
function hasSecureRel(node, allowReferrer, warnOnSpreadAttributes, spreadAttributeIndex) {
  const relIndex = node.attributes.findLastIndex((attr) => attr.type === 'JSXAttribute' && attr.name.name === 'rel');
  const targetIndex = node.attributes.findLastIndex(
  ...
  create(context) {
    ...
    return {
      JSXOpeningElement(node) {
        const targetIndex = node.attributes.findLastIndex((attr) => attr.name && attr.name.name === 'target');
        const spreadAttributeIndex = node.attributes.findLastIndex((attr) => attr.type === 'JSXSpreadAttribute');
        ...
        if (linkComponents.has(node.name.name)) {
          ...
          const hasDangerousLink =
            hasExternalLink(node, linkAttributes, warnOnSpreadAttributes, spreadAttributeIndex) ||
