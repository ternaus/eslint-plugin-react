...
import getTokenBeforeClosingBracket from '../util/getTokenBeforeClosingBracket.js';
...
function validateClosingSlash(context, node, option) {
  ...
  if (node.selfClosing) {
    ...
    adjacent = !eslintUtil.isSpaceBetweenTokens(sourceCode, lastTokens[0], lastTokens[1]);
    ...
    adjacent = !eslintUtil.isSpaceBetweenTokens(sourceCode, firstTokens[0], firstTokens[1]);
...
function validateBeforeSelfClosing(context, node, option) {
  ...
  const adjacent = !eslintUtil.isSpaceBetweenTokens(sourceCode, leftToken, closingSlash);
...
function validateAfterOpening(context, node, option) {
  ...
  const adjacent = !eslintUtil.isSpaceBetweenTokens(sourceCode, openingToken, node.name);
...
function validateBeforeClosing(context, node, option) {
  ...
  if (!node.selfClosing) {
    const sourceCode = getSourceCode(context);
    const leftToken =
      option === 'proportional-always' ? getTokenBeforeClosingBracket(node) : sourceCode.getLastTokens(node, 2)[0];
    ...
    const adjacent = !eslintUtil.isSpaceBetweenTokens(sourceCode, leftToken, closingToken);
