...
const getScope = requiredModule0.getScope;
  ...
  create(context) {
    ...
    function isStateReference(node) {
      ...
      const isDirectStateReference =
        node.type === 'MemberExpression' && isThisExpression(node.object) && node.property.name === 'state';

      const isAliasedStateReference =
        node.type === 'Identifier' && classInfo.aliases && classInfo.aliases.has(node.name);
    ...
    return {
      ...
      FunctionExpression(node) {
        ...
        if ('key' in parent && 'name' in parent.key && parent.key.name === 'getInitialState') {
