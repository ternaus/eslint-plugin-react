    ...
    function validateCurlys(curlys, expression) {
      ...
      if (hasLeftNewline && !needsNewlines) {
        report(context, messages.unexpectedAfter, 'unexpectedAfter', {
          node: leftCurly,
          fix(fixer) {
            return getText(context).slice(leftCurly.range[1], tokenAfterLeftCurly.range[0]).trim()
      ...
      if (hasRightNewline && !needsNewlines) {
        report(context, messages.unexpectedBefore, 'unexpectedBefore', {
          node: rightCurly,
          fix(fixer) {
            return getText(context).slice(tokenBeforeRightCurly.range[1], rightCurly.range[0]).trim()
