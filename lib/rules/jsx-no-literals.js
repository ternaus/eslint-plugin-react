 ...
 * @property {string[]} [restrictedAttributes]
 * @property {Record<string, RawOverrideConfig>} [elementOverrides]
 */

/**
 * @typedef {object} RawOverrideConfig
 ...
 * @property {boolean} ignoreProps
 * @property {boolean} noAttributeStrings
 * @property {Set<string>} restrictedAttributes
 */
...
function normalizeElementConfig(config) {
  return {
    ...
    allowedStrings: config.allowedStrings ? new Set(config.allowedStrings.map(trimIfString)) : new Set(),
    ...
    restrictedAttributes: config.restrictedAttributes
      ? new Set(config.restrictedAttributes.map(trimIfString))
      : new Set(),
  };
}
...
function normalizeConfig(config) {
  ...
  if (config.elementOverrides) {
    normalizedConfig.elementOverrides = Object.fromEntries(
      Object.entries(config.elementOverrides).flatMap(([elementName, rawElementConfig]) => {
...
const elementOverrides = {
  ...
  patternProperties: {
    [reOverridableElement.source]: {
      type: 'object',
      properties: Object.assign({ applyToNestedElements: { type: 'boolean' } }, commonPropertiesSchema),
    },
  },
};
...
const exported = {
  ...
  create(context) {
    ...
    function isViableTextNode(node, resolvedConfig) {
      const textValues = [trimIfString(node.raw), trimIfString(node.value)];
      if (textValues.some((value) => resolvedConfig.allowedStrings.has(value))) {
        return false;
      }
    ...
    return Object.assign(
        ...
        ? {
            ...
            ImportDeclaration(node) {
              node.specifiers
                ...
                .forEach((specifier) => {
                  renamedImportMap.set((specifier.local || specifier.imported).name, specifier.imported.name);
                });
            },
            ...
            VariableDeclaration(node) {
              node.declarations
                .filter(
                  (d) => d.type === 'VariableDeclarator' && isRequireStatement(d.init) && d.id.type === 'ObjectPattern',
                )
                .forEach((declaration) => {
        ...
        : false,
      {
        ...
        JSXAttribute(node) {
          const isLiteralString = node.value && node.value.type === 'Literal' && typeof node.value.value === 'string';
        ...
        TemplateLiteral(node) {
          ...
          if (isParentJSXExpressionCont) {
            ...
            if (resolvedConfig.noStrings && (isParentJSXElement || !resolvedConfig.ignoreProps)) {
