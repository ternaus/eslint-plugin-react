...
import requiredModule2 from '../util/ast.js';

const isParenthesized = requiredModule2.isParenthesized;
...
function isInvalidTernaryAlternate(context, node) {
  ...
  return node.type === 'Literal' && TERNARY_INVALID_ALTERNATE_VALUES.includes(node.value);
}
...
function ruleFixer(context, fixStrategy, fixer, reportedNode, leftNode, rightNode) {
  ...
  if (fixStrategy === COERCE_STRATEGY) {
    ...
    if (
      ...
      isFalseLiteral(rightNode.parent.consequent)
    ) {
      const consequentVal = getText(context, rightNode.parent.consequent);
...
const exported = {
  ...
  create(context) {
    ...
    const [fixStrategy] = validStrategies;
