...
import isCreateContext from '../util/isCreateContext.js';
...
const exported = {
  ...
  create: Components.detect((context, components, utils) => {
    ...
    function reportMissingDisplayName(component) {
      if (isNestedMemo(component.node)) {
        return;
      }
    ...
    function hasTranspilerName(node) {
      ...
      const namedClass =
        (node.type === 'ClassDeclaration' || node.type === 'ClassExpression') && node.id && !!node.id.name;

      const namedFunctionDeclaration =
        (node.type === 'FunctionDeclaration' || node.type === 'FunctionExpression') && node.id && !!node.id.name;
      ...
      if (node.type === 'VariableDeclaration') {
        return node.declarations.some((decl) => {
          ...
          if (decl.id.type === 'ArrayPattern') {
            return decl.id.elements.some((el) => el && el.type === 'Identifier' && el.name === name);
          }

          // const { name } = ...
          if (decl.id.type === 'ObjectPattern') {
            return decl.id.properties.some((prop) => prop.type === 'Property' && prop.key && prop.key.name === name);
          }
        ...
        if (
          ...
          const isParamShadowed = currentNode.params.some((param) => {
            ...
            if (param.type === 'ArrayPattern') {
              return param.elements.some((el) => el && el.type === 'Identifier' && el.name === identifierName);
            }
            return false;
          });
    ...
    function isShadowedComponent(node) {
      ...
      if (node.callee.type === 'MemberExpression' && node.callee.object.name === 'React') {
    ...
    return {
      ...
      'Program:exit'() {
        ...
        if (checkContextObjects) {
          ...
          [...contextObjects.values()]
            .filter((contextObj) => !contextObj.hasDisplayName)
            .forEach((contextObj) => reportMissingContextDisplayName(contextObj));
        }
      },
    };
  }),
