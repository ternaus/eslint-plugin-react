      ...
      Object.keys(childrenGroupedByLine).forEach((_line) => {
        ...
        childrenGroupedByLine[line].forEach((child, i) => {
          ...
          function spaceBetweenPrev() {
            return (
              ((prevChild.type === 'Literal' || prevChild.type === 'JSXText') && / $/.test(prevChild.raw)) ||
              ((child.type === 'Literal' || child.type === 'JSXText') && /^ /.test(child.raw)) ||
              eslintUtil.isSpaceBetweenTokens(getSourceCode(context), prevChild, child)
            );
          }

          function spaceBetweenNext() {
            return (
              ((nextChild.type === 'Literal' || nextChild.type === 'JSXText') && /^ /.test(nextChild.raw)) ||
              ((child.type === 'Literal' || child.type === 'JSXText') && / $/.test(child.raw)) ||
              eslintUtil.isSpaceBetweenTokens(getSourceCode(context), child, nextChild)
            );
          }
