...
import requiredModule0 from '../util/ast.js';

const isParenthesized = requiredModule0.isParenthesized;

import docsUrl from '../util/docsUrl.js';
import requiredModule1 from '../util/eslint.js';

const getSourceCode = requiredModule1.getSourceCode;

import getMessageData from '../util/message.js';
      ...
      'FunctionExpression, ArrowFunctionExpression'(node) {
        ...
        if (node.params.length === 1) {
          report(context, messages.missingRefParameter, 'missingRefParameter', {
            node,
            suggest: [
              Object.assign(getMessageData('addRefParameter', messages.addRefParameter), {
                fix(fixer) {
                  const param = node.params[0];
                  // If using shorthand arrow function syntax, add parentheses around the new parameter pair
                  const shouldAddParentheses =
                    node.type === 'ArrowFunctionExpression' && !isParenthesized(context, param);
                  return [].concat(
              ...
              Object.assign(getMessageData('removeForwardRef', messages.removeForwardRef), {
                fix(fixer) {
