    ...
    return {
        ...
        if (
          ...
          while (current.type !== 'Program') {
            ...
            if (current.type === 'MethodDefinition') {
              ...
              break;
            } else if (current.type === 'FunctionExpression' && 'key' in current.parent && current.parent.key) {
              methods.push({
      ...
      ObjectPattern(node) {
        const isDerivedFromThis =
          'init' in node.parent && node.parent.init && node.parent.init.type === 'ThisExpression';
