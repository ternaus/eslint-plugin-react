...
const isAssignmentLHS = requiredModule0.isAssignmentLHS;
...
const exported = {
  ...
  create: Components.detect((context, components, utils) => {
    ...
    function handleSFCUsage(node) {
      ...
      const isPropUsed =
        ((propsName && node.object.name === propsName) || (contextName && node.object.name === contextName)) &&
        ...
        if (
          ...
          report(context, messages.destructureInSignature, 'destructureInSignature', {
            ...
            fix(fixer) {
              ...
              return [fixer.replaceTextRange(replaceRange, getText(context, node.id)), fixer.remove(node.parent)];
            },
          });
        }
      },
