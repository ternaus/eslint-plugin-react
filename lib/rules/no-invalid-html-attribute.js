...
const KNOWN_ATTRIBUTES = new Set([
  ...
  ...Object.values(REACT_DOM_ATTRIBUTE_OVERRIDES).flatMap((attributes) => Object.keys(attributes)),
]);
...
function isValidNavigableTarget(value) {
  ...
  // WHATWG only excludes a target name containing both ASCII tab/newline and "<".
  return !(ASCII_TAB_OR_NEWLINE_PATTERN.test(value) && value.includes('<'));
}

function getAttributeDefinition(element, elementAttributes, attribute) {
  if (Object.hasOwn(elementAttributes, attribute)) {
    return elementAttributes[attribute];
  }
  if (Object.hasOwn(REACT_DOM_ATTRIBUTE_OVERRIDES[element] ?? {}, attribute)) {
    return REACT_DOM_ATTRIBUTE_OVERRIDES[element][attribute];
  }
...
const exported = {
  ...
  create(context) {
    function checkAttribute(element, elementAttributes, attributeName, valueNode, reportNode) {
      ...
      const definition = getAttributeDefinition(element, elementAttributes, attribute);
