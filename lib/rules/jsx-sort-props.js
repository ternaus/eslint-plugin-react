...
import propName from 'jsx-ast-utils/propName.js';
...
function hasAmbiguousComment(node, context) {
  ...
  return comments.some(
    (comment) =>
      ...
      !node.attributes.some(
        (attribute) => comment.range[0] >= attribute.range[0] && comment.range[1] <= attribute.range[1],
      ),
  );
}

function generateFixerFunction(node, context, reservedList) {
  if (hasAmbiguousComment(node, context)) {
    return null;
  }
