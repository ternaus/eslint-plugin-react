...
function hasLowercaseInitial(parentName) {
  const initial = parentName[0] === '_' ? parentName[1] : parentName[0];
  return !initial || initial === initial.toLowerCase();
}
  ...
  create: Components.detect((context, components, utils) => {
    ...
    function validate(node) {
      ...
      if (parentComponent) {
        ...
        if (parentName && hasLowercaseInitial(parentName)) {
          return;
        }
