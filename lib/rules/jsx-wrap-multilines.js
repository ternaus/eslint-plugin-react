...
import reportC from '../util/report.js';
...
const exported = {
  ...
  create(context) {
    ...
    function trimTokenBeforeNewline(_node, tokenBefore) {
    ...
    function check(node, type) {
      ...
      if (option === 'never' && isParenthesized(context, node)) {
        ...
        report(node, 'extraParens', (fixer) =>
          fixer.replaceTextRange([tokenBefore.range[0], tokenAfter.range[1]], getText(context, node)),
        );
      }
    }
