...
function getDOMPropertyNames() {
  return [].concat(DOM_PROPERTY_NAMES_TWO_WORDS, DOM_PROPERTY_NAMES_ONE_WORD, REACT_ON_PROPS, 'precedence');
}
...
function isValidHTMLTagInJSX(childNode) {
  ...
  if (tagConvention.test(childNode.parent.name.name)) {
    return !childNode.parent.attributes.some(
      (attrNode) =>
        attrNode.type === 'JSXAttribute' && attrNode.name.type === 'JSXIdentifier' && attrNode.name.name === 'is',
