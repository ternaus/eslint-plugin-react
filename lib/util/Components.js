...
import defaultPropsUtil from './defaultProps.js';
...
import jsxUtil from './jsx.js';
import pragmaUtil from './pragma.js';
import propTypesUtil from './propTypes.js';
import usedPropTypesUtil from './usedPropTypes.js';
...
function mergeRules(rules) {
  ...
  return Object.fromEntries(
    Array.from(handlersByKey, (entry) => [
      entry[0],
  ...
  const utils = {
    ...
    getStatelessComponent(node) {
      ...
      if (node.type === 'FunctionExpression' || node.type === 'ArrowFunctionExpression') {
        ...
        const isModuleExportsAssignment =
          isPropertyAssignment && parent.left.object.name === 'module' && parent.left.property.name === 'exports';
    ...
    isReactHookCall(node, expectedHookNames) {
      ...
      const reactHookImportNames =
        ...
        Object.fromEntries(
          reactHookImportSpecifiers.map((specifier) => [specifier.local.name, specifier.imported.name]),
        );
      ...
      const isPotentialHookCall =
        reactHookImportNames && node.callee.type === 'Identifier' && node.callee.name.match(USE_HOOK_PREFIX_REGEX);
      ...
      const potentialHookReference =
        isPotentialHookCall &&
        scope.references &&
        scope.references.find((reference) => reactHookImportNames[reference.identifier.name]);
      ...
      const isHookShadowed =
        isPotentialHookCall &&
        hookResolvedDefs &&
        hookResolvedDefs.some((hookDef) => hookDef.name.name === localHookName && hookDef.type !== 'ImportBinding');

      const isHookCall =
        (isPotentialReactHookCall && !isReactShadowed) || (isPotentialHookCall && localHookName && !isHookShadowed);
...
const exported = Object.assign(Components, {
  detect(rule) {
