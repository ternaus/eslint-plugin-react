...
function getPropertyNameNode(node) {
  if (node.key || node.type === 'MethodDefinition' || node.type === 'Property') {
    return node.key;
  }
...
function isTSInterfaceDeclaration(node) {
  ...
  return (
    (node.type === 'ExportNamedDeclaration' && node.declaration ? node.declaration.type : node.type) ===
