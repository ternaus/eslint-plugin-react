...
function isModuleCall(context, node, moduleName, name) {
  ...
  if (
    ...
    getVariable(context, identifier)?.references.some((reference) => reference.isWrite() && !reference.init)
  ) {
    return false;
  }
  if (callee.type === 'Identifier') {
    return isNamedImport(context, callee, moduleName, name);
  }
  return (
    ...
    isModuleObject(context, callee.object, moduleName)
  );
}

function isReactCall(context, node, name) {
  return isModuleCall(context, node, 'react', name);
}

function isReactJsxElementName(context, node, name) {
  if (node.type === 'JSXIdentifier') {
    return isNamedImport(context, node, 'react', name);
  }
  return (
    ...
    isModuleObject(context, node.object, 'react')
  );
}

function getConstantInitializer(context, identifier) {
  const definition = getVariable(context, identifier)?.defs?.[0];
  if (
