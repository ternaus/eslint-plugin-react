...
import isFirstLetterCapitalized from './isFirstLetterCapitalized.js';
import propsUtil from './props.js';
import propWrapperUtil from './propWrapper.js';
import variableUtil from './variable.js';
...
function isFunctionType(node) {
  ...
  return (
    nodeType === 'FunctionDeclaration' || nodeType === 'FunctionExpression' || nodeType === 'ArrowFunctionExpression'
  );
}
...
const exported = function propTypesInstructions(context, components, utils) {
  ...
  function buildReactDeclarationTypes(value, parentName, rootNode) {
    ...
    if (value && value.type === 'MemberExpression' && value.property && value.property.name === 'isRequired') {
      value = value.object;
    }
  ...
  function isValidReactGenericTypeAnnotation(annotation) {
    ...
    if (typeNameNode) {
      ...
      } else if (typeNameNode.right && typeNameNode.right.name) {
        // if React.FC<Props>
  ...
  function isReactGenericTypeAnnotation(annotation) {
    return !!(
      annotation &&
      (astUtil.isTSTypeReference(annotation) || astUtil.isTSInterfaceHeritage(annotation)) &&
  ...
  function isReactComponentTypeAlias(annotation) {
    ...
    return getSourceCode(context).ast.body.some((node) => {
      ...
      if (astUtil.isTSTypeAliasDeclaration(declaration)) {
        return isReactGenericTypeAnnotation(declaration.typeAnnotation);
      }

      return astUtil.isTSInterfaceDeclaration(declaration) && declaration.extends?.some(isReactGenericTypeAnnotation);
    });
  }
  ...
  class DeclarePropTypesForTSTypeAnnotation {
    ...
    searchDeclarationByName(node) {
      ...
      if (astUtil.isTSTypeReference(node)) {
        ...
        if (shouldTraverseTypeParams && nodeTypeArguments && nodeTypeArguments.length !== 0) {
          ...
          if (
            leftMostName === 'React' &&
            (rightMostName === 'HTMLAttributes' || rightMostName === 'HTMLElement' || rightMostName === 'HTMLProps')
          ) {
          ...
          const idx =
            genericTypeParamIndexWherePropsArePresent[leftMostName !== rightMostName ? rightMostName : importedName];
    ...
    convertReturnTypeToPropTypes(node, rootNode) {
      ...
      if (nodeTypeArguments) {
        if (nodeTypeArguments.params.length === 1) {
          const returnType = nodeTypeArguments.params[0];
    ...
    endAndStructDeclaredPropTypes() {
      ...
      this.foundDeclaredPropertiesList.forEach((tsInterfaceBody) => {
        if (
          tsInterfaceBody &&
          (tsInterfaceBody.type === 'TSPropertySignature' || tsInterfaceBody.type === 'TSMethodSignature')
        ) {
          let accessor = 'name';
    ...
    switch (propTypes && propTypes.type) {
      ...
      case 'MemberExpression': {
        ...
        if (propTypes && propTypes.parent && propTypes.property) {
          ...
          const types = buildReactDeclarationTypes(propTypes.parent.right, parentProp, rootNode);
          ...
          while (n) {
            if (
              (n.type === 'AssignmentExpression' && propsUtil.isPropTypesDeclaration(n.left)) ||
              ((n.type === 'ClassProperty' || n.type === 'PropertyDefinition' || n.type === 'Property') &&
      ...
      case 'CallExpression': {
        if (
          propWrapperUtil.isPropWrapperFunction(context, getText(context, propTypes.callee)) &&
          propTypes.arguments &&
  ...
  function markAnnotatedFunctionArgumentsAsDeclared(node, rootNode) {
    ...
    if (isNodeAnnotated) {
      ...
      if (annotation && annotation.type !== 'TSTypeReference' && propsUtil.getTypeArguments(annotation) == null) {
        return;
      }

      if (!isValidReactGenericTypeAnnotation(annotation) && !isReactComponentTypeAlias(annotation)) return;
  ...
  function resolveSuperParameterPropsType(node) {
    const parameters = propsUtil.getSuperTypeArguments(node);
    // The current Flow form puts Props first. The legacy three-parameter form
    // remains identifiable from its syntax without resolving a package version.
    const propsParameterPosition = parameters.params.length <= 2 ? 0 : 1;
