...
const getVariable = variableUtil.getVariable;
...
function inLifeCycleMethod(context, node) {
  ...
  while (scope) {
    if (scope.block && scope.block.parent && scope.block.parent.key) {
      ...
      if (ASYNC_SAFE_LIFE_CYCLE_METHODS.indexOf(name) >= 0) {
        return true;
      }
    }
    scope = scope.upper;
...
function isNodeALifeCycleMethod(node) {
  if (node.key) {
    ...
    if (ASYNC_SAFE_LIFE_CYCLE_METHODS.indexOf(nodeKeyName) >= 0) {
      return true;
    }
  }
...
function isSetStateUpdater(node) {
  const unwrappedParentCalleeNode =
    astUtil.isCallExpression(node.parent) && astUtil.unwrapTSAsExpression(node.parent.callee);
...
/**
 * Checks whether a variable reference resolves to a parameter of an inline callback.
 *
...
function isParamOfInlineCallback(context, node, name, utils) {
  ...
  if (definition.type !== 'Parameter' || !definition.node || utils.getStatelessComponent(definition.node)) {
    return false;
  }

  let child = definition.node;
  ...
  return (
    (!!parent &&
      (astUtil.isCallExpression(parent) || parent.type === 'NewExpression') &&
 ...
 * @returns {boolean}
 */
function isPropTypesUsageByMemberExpression(context, node, utils) {
  ...
  if (isInClassComponent(context, node)) {
    ...
    if (
      ...
      (inLifeCycleMethod(context, node) || astUtil.inConstructor(context, node))
    ) {
      return true;
    }
  ...
  return (
    unwrappedObjectNode.name === 'props' &&
 ...
 */
function getPropertyName(context, node, utils) {
    ...
    switch (property.type) {
      ...
      case 'Literal': {
        ...
        if (typeof property.value === 'number') {
          if (isPropTypesUsageByMemberExpression(context, node, utils)) {
            return property.raw;
          }
        }
        if (node.computed) {
...
const exported = function usedPropTypesInstructions(context, components, utils) {
  ...
  function markPropTypesAsUsed(node, parentNames) {
    ...
    if (astUtil.isMemberExpression(node)) {
      name = getPropertyName(context, node, utils);
      if (name) {
        ...
        if (astUtil.isVariableDeclarator(parent) && parent.id.type === 'ObjectPattern') {
        ...
        // const a = props.a
        if (astUtil.isVariableDeclarator(parent) && parent.id.type === 'Identifier') {
  ...
  function markDestructuredFunctionArgumentsAsUsed(node) {
    ...
    const destructuring =
      param &&
      (param.type === 'ObjectPattern' || (param.type === 'AssignmentPattern' && param.left.type === 'ObjectPattern'));
